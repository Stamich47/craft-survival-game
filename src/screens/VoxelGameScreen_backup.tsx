...
export const VoxelGameScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (!isLoaded) {
      dispatch(generateWorld({ width: 24, height: 24, depth: 8 }));
    }
  }, [dispatch, isLoaded]);

  // Track previous gesture position for incremental movement
  const previousGesture = useRef({ x: 0, y: 0 });

  // Simple pan responder - original working version
  const panResponder = PanResponder.create({
    onStartShouldSetPanResponder: () => true,
    onMoveShouldSetPanResponder: () => true,
    onPanResponderGrant: (evt, gestureState) => {
      // Store initial position when gesture starts
      previousGesture.current = { x: gestureState.dx, y: gestureState.dy };
    },
    onPanResponderMove: (evt, gestureState) => {
      // Calculate incremental movement since last update
      const deltaX = gestureState.dx - previousGesture.current.x;
      const deltaY = gestureState.dy - previousGesture.current.y;
      
      // Update camera with larger incremental movements for more responsive feel
      dispatch(moveCamera({ deltaX: deltaX * 2.0, deltaY: deltaY * 2.0 }));
