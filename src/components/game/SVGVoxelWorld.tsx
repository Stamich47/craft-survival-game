...
// Memoized individual voxel/cube component using SVG
const SVGVoxelCube = memo<VoxelProps>(
  ({ x, y, z, type, tileSize, screenX, screenY }) => {
    const cubeColors = {
...
export const SVGVoxelWorld = memo<SVGVoxelWorldProps>(
  ({ world, tileSize = 32, offsetX = 0, offsetY = 0 }) => {
    // Convert world data to renderable voxels (frustum culling disabled for debugging)
        ...
        for (let y = 0; y < world[x].length; y++) {
          for (let z = 0; z < world[x][y].length; z++) {
            ...
            if (voxelType !== 0) {
              ...
              const screenX = isoPos.x + screenWidth / 2 + offsetX;
              const screenY = isoPos.y + screenHeight / 2 + offsetY;

              // Render all blocks for now (no frustum culling)
              result.push({
                x,
                y,
                z,
